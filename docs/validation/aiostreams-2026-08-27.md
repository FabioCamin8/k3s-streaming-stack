...
The unauthenticated `GET /api/v1/user` probe returned 400 before a user
configuration existed; it is not an acceptance gate. A provider-free user
configuration was created in the earlier smoke sequence, and its configured
manifest remained usable across Pod recreation. No provider or debrid
...
- The first post-reboot verifier attempt was a timing race: it ran before K3s
  ...
  loss occurred.
- The verifier now waits for the target namespace for up to `WAIT_SECONDS`
  before evaluating dependent resources, while still failing closed on timeout.
...
- Pod recreation and post-recreation persistence passed before the final reboot
  proof; the application data remains node-local and is not a backup.
