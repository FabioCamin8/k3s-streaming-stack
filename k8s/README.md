...
verified. The cert-manager platform baseline, AIOStreams workload, and Remux
workload are implemented as separate, small layers. Remux's private hostname,
manifest path, and API token are supplied outside Git.
...
- [`remux/`](remux/README.md): one-replica Remux workload, private rendering,
  and AIOStreams addon configuration.
